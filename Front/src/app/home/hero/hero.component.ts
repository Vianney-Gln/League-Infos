...
import { concatMap, filter, finalize, forkJoin, map, Subscription, switchMap, tap } from 'rxjs';
...
export class HeroComponent implements OnInit, OnDestroy {
  ...
  getMostrecentChampionSubscription: Subscription | null = null;
  ...
  urlBackgroundMostBannedChampion: WritableSignal<string> = signal('');

  NB_PREVIOUS_VERSIONS = 20;
  INTERVAL_CAROUSSEL = 8000;
  ...
  private getDataBestSoloqPlayer(): void {
    ...
    this.leagueChallengerSoloQSubscription = this.playersService
      ...
      .pipe(
        ...
        filter(this.hasValidEntries),
        concatMap((leagueListDTO) => {
  ...
  private hasValidEntries(dto: LeagueListDTO | undefined): dto is LeagueListDTO {
    return !!dto && Array.isArray(dto.entries) && dto.entries.length > 0;
  }
  ...
  ngOnDestroy(): void {
    ...
    if (this.getMostrecentChampionSubscription) {
      this.getMostrecentChampionSubscription.unsubscribe();
    }
  }
}
