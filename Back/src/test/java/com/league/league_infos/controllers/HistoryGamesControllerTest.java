...
class HistoryGamesControllerTest {
    ...
    @Test
    @DisplayName("Should return a match history by gameId")
    void getGamesHistory_success() throws Exception {
        // GIVEN
        MatchDTO matchDTO = new MatchDTO.Builder()
                .metadata(new MetadataDTO.Builder()
                        .matchId("200")
                ...
                .info(new InfoMatchDTO.Builder()
                        ...
                        .build())
                .build();
        when(historyPersistence.findMatchByGameId(anyLong())).thenReturn(matchDTO);

        // WHEN
        mockMvc.perform(get("/game-history/200")
                ).andExpect(status().isOk())
                .andExpect(content().json("""
                        {"metadata":{
                            "dataVersion":"dataVersion",
        ...
        // THEN
        verify(historyPersistence, times(1)).findMatchByGameId(200L);
    }
}
